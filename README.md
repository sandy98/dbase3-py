...
```
or, even better, if dbase3_py is installed using pip, it will install dbfview as a script, thus:
```bash
dbfview <dbf_file>
```
...
```
or, even better, if dbase3_py is installed using pip, it will install dbftest as a script, thus:
```bash
dbftest  [-r|-d]
```
